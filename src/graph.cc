...
class RecomputeOutputsDirtyCache {
  ...
  class CachedLogEntry {
   public:
    ...
    /// check for nullptr
    bool is_valid() const { return entry_ && entry_ != kUnevaluated; }
   ...
   private:
    // Sentinel meaning "not yet evaluated". nullptr means "evaluated, not found".
    static BuildLog::LogEntry* const kUnevaluated;
    BuildLog::LogEntry* entry_ = kUnevaluated;
...
static BuildLog::LogEntry kUnevaluatedStorage{""};
BuildLog::LogEntry* const
    RecomputeOutputsDirtyCache::CachedLogEntry::kUnevaluated =
        &kUnevaluatedStorage;

bool RecomputeOutputsDirtyCache::CachedLogEntry::LookupByOutput(
    const BuildLog* buildLog, const Node* output) {
  if (entry_ != kUnevaluated) {
